...
export const smartAuth = async (req, res, next) => {
    try {
        ...
        // Enhanced Debug Logs
        console.log(`[Auth] Attempt - Cookie: ${!!req.cookies?.accessToken}, Bearer: ${!!bearerToken}, API-Key: ${!!apiKeyHeader}`);
        if (!accessToken && !apiKeyHeader) {
            console.warn("[Auth] No credentials provided in request.");
        }
