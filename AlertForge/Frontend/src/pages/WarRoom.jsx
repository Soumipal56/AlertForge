...
const upsertIncident = (currentIncidents, incident) => {
  ...
  const existingIndex = currentIncidents.findIndex(
          ...
          <div className="flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between">
            <div>
              ...
              <p className="mt-3 max-w-2xl text-sm leading-6 text-slate-300">
                Loaded from the backend incidents API and updated instantly
            ...
            <div className="grid gap-3 rounded-2xl border border-white/10 bg-black/20 p-4 text-sm text-slate-300 sm:grid-cols-4">
              <div>
                ...
                <div
                  className={`mt-1 font-medium ${socketStatus === "connected" ? "text-emerald-400" : "text-red-400"}`}
                >
                  {socketStatus}
                </div>
              </div>
            ...
            <div className="mb-4 flex items-center justify-between">
              <div>
                ...
                <p className="text-sm text-slate-400">
                  New incidents are prepended automatically.
                </p>
              </div>
              {loading ? (
                  ...
                  <div className="flex flex-col gap-4 md:flex-row md:items-start md:justify-between">
                    <div className="space-y-2">
                      <div className="flex flex-wrap items-center gap-2">
                        <span
                          className={`rounded-full border px-3 py-1 text-xs font-medium ${severityStyles[incident.severity] || severityStyles.medium}`}
                        >
                          {incident.severity}
                        </span>
                        <span
                          className={`rounded-full border px-3 py-1 text-xs font-medium ${statusStyles[incident.status] || statusStyles.open}`}
                        >
                          {incident.status}
                        </span>
                      ...
                      <p className="text-xs text-slate-500">
                        Updated:{" "}
                        {incident.updatedAt || incident.createdAt || "just now"}
                      </p>
                    </div>
                    <button
                      type="button"
          ...
          <aside className="space-y-6">
            <section className="rounded-3xl border border-white/10 bg-white/5 p-5">
              ...
              <p className="mt-1 text-sm text-slate-400">
                Uses `POST /api/incidents` with your API key.
              </p>
              ...
              <form onSubmit={handleCreateIncident} className="mt-4 space-y-4">
                ...
                />
                <p className="text-xs text-slate-500">
                  The key is stored locally so the dashboard can fetch incidents
                  again after refresh.
                </p>
                <input
                  type="text"
            ...
            <section className="rounded-3xl border border-white/10 bg-white/5 p-5">
              ...
              <p className="mt-1 text-sm text-slate-400">
                Socket event log for incident activity.
              </p>
