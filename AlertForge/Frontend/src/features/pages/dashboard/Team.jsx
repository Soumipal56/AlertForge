...
const INITIAL_MEMBERS = [
  ...
  },
  {
    ...
    email: "omar@acme.com",
    initials: "OF",
    color: "bg-pink-900/80 text-pink-200",
    role: "Responder",
    online: false,
...
const roleConfig = {
  Admin: {
    ...
    description:
      "Full access — manage team, billing, integrations, and all incidents",
    can: [
  ...
  Responder: {
    ...
    description:
      "Can respond to and manage incidents — no team or billing access",
    can: [
  ...
  Viewer: {
    ...
    description:
      "Read-only access — can view incidents and status but cannot make changes",
    can: [
      "View all incidents",
      "View timelines",
      "View status page",
    ...
    <div className="grid grid-cols-[2fr_1fr_1fr_1fr_auto] gap-4 items-center border-b border-zinc-800 px-0 py-4 text-sm transition-colors hover:bg-zinc-950/60">
      ...
      <div className="flex items-center gap-3 min-w-0">
        <div className="relative shrink-0">
          <div
            className={`flex h-8 w-8 items-center justify-center rounded-full text-[11px] font-bold ${member.color}`}
          >
            {member.initials}
          </div>
          <span
        ...
        <div className="min-w-0">
          <div className="flex items-center gap-2">
            ...
            {isCurrentUser && (
              <span className="rounded-full bg-zinc-800 px-2 py-0.5 text-[10px] text-zinc-500 font-medium shrink-0">
                You
              </span>
            )}
          </div>
      ...
      <div>
        <span
          className={`inline-flex items-center gap-1.5 rounded-md px-2 py-1 text-xs font-semibold ${roleCfg.badge}`}
        >
      ...
      <div className="flex items-center gap-1.5">
        <span
          className={`h-1.5 w-1.5 rounded-full ${member.online ? "bg-emerald-500" : "bg-zinc-600"}`}
        />
        <span
          className={`text-xs ${member.online ? "text-emerald-400" : "text-zinc-600"}`}
        >
        ...
        {!isCurrentUser ? (
          <DropdownMenu>
            <DropdownMenuTrigger asChild>
              <button
                type="button"
                className="flex h-7 w-7 items-center justify-center rounded-md text-zinc-600 hover:bg-zinc-800 hover:text-zinc-300 transition-colors outline-none"
              >
                ...
                >
                  {ROLES.map((r) => {
                    ...
                    return (
                      ...
                      >
                        <RoleIcon
                          className={`size-3 ${roleConfig[r].iconClass}`}
                        />
                        {r}
                      </DropdownMenuRadioItem>
                    );
...
function InviteRow({ invite, onResend, onRevoke }) {
  const roleCfg = roleConfig[invite.role];
  ...
  return (
    <div className="grid grid-cols-[2fr_1fr_1fr_auto] gap-4 items-center border-b border-zinc-800 px-0 py-4 text-sm">
      {/* Email */}
      <div className="flex items-center gap-3 min-w-0">
        <div className="flex h-8 w-8 items-center justify-center rounded-full border border-dashed border-zinc-700 bg-zinc-900 shrink-0">
          <Mail className="size-3.5 text-zinc-600" />
        </div>
        <div className="min-w-0">
          <p className="text-sm text-zinc-400 truncate">{invite.email}</p>
          <p className="text-[11px] text-zinc-700 mt-0.5">Invite pending</p>
        </div>
      </div>
      ...
      {/* Sent + expires */}
      <div>
        <p className="text-[11px] text-zinc-600">Sent {invite.sentAt}</p>
        <p className="text-[11px] text-zinc-700 mt-0.5 flex items-center gap-1">
          <Clock className="size-3" /> Expires in {invite.expiresIn}
        </p>
      </div>
      ...
      <div className="flex items-center gap-2">
        <button
          ...
          className={`flex items-center gap-1.5 rounded-md border px-2.5 py-1.5 text-xs font-medium transition-all duration-200
            ${
              resent
        ...
        <button
          ...
          className="flex h-7 w-7 items-center justify-center rounded-md text-zinc-700 hover:bg-zinc-800 hover:text-red-400 transition-colors"
        >
...
function RoleLegendCard({ role }) {
  ...
  return (
    <div className="rounded-lg border border-zinc-800 bg-zinc-950 p-4 flex flex-col gap-3">
      <div className="flex items-center gap-2">
        <div
          className={`flex h-7 w-7 items-center justify-center rounded-lg bg-zinc-900 border border-zinc-800`}
        >
          <cfg.icon className={`size-3.5 ${cfg.iconClass}`} />
        </div>
        <span
          className={`rounded-md px-2 py-0.5 text-xs font-semibold ${cfg.badge}`}
        >
          {role}
        </span>
      </div>
      ...
      >
        <SheetHeader>
          ...
          <SheetDescription className="text-zinc-400">
            Send an invite link to a new member. They'll be prompted to create
            an account.
          </SheetDescription>
        </SheetHeader>
            ...
            <div className="flex flex-col gap-2">
              {ROLES.map((r) => {
                ...
                return (
                  ...
                  >
                    <div
                      className={`flex h-7 w-7 shrink-0 items-center justify-center rounded-lg bg-zinc-950 border border-zinc-800 mt-0.5`}
                    >
                      <cfg.icon className={`size-3.5 ${cfg.iconClass}`} />
                    </div>
...
export default function Team() {
  ...
  // Current user is always id 1 (Mustafa) for mock
  ...
  const handleResend = (id) => {
    // In real app — call API to resend invite
        ...
        <div className="rounded-lg border border-zinc-800 bg-zinc-950 overflow-hidden mb-5">
          <div className="flex items-center justify-between px-5 py-3 border-b border-zinc-800">
            <div className="flex items-center gap-2">
              ...
              <span className="rounded-full bg-orange-950/60 border border-orange-900/50 px-2 py-0.5 text-[10px] font-semibold text-orange-300">
                {invites.length}
              </span>
            </div>
            <span className="text-[11px] text-zinc-600">
          ...
          {/* Column labels */}
          <div className="grid grid-cols-[2fr_1fr_1fr_auto] gap-4 border-b border-zinc-800 px-5 py-2.5 text-[11px] font-semibold uppercase tracking-wide text-zinc-600">
            <span>Email</span>
            <span>Role</span>
            <span>Sent</span>
            <span className="sr-only">Actions</span>
